...
def _values_to_words(values, loss_weights, dic):
    values = tf.constant(values)
    seqs = []
    for idx, line in enumerate(values):
        seq = []
        mask = loss_weights[idx]
        for i, w_idx in enumerate(line):
            if mask[i] is False:
                break
            seq.append(dic[w_idx])
        seqs.append(seq)
    return seqs
...
def main(_):
    ...
    if FLAGS.use_bpe:
        trainset, trn_node, trn_in_neigh, trn_out_neigh, trn_sent = G2S_data_stream.read_amr_file(FLAGS.train_bpe_path, True)
    else:
        trainset, trn_node, trn_in_neigh, trn_out_neigh, trn_sent = G2S_data_stream.read_amr_file(FLAGS.train_path, False)
    print('Number of training samples: {}'.format(len(trainset)))

    print('Loading dev set.')
    if FLAGS.use_bpe:
        devset, tst_node, tst_in_neigh, tst_out_neigh, tst_sent = G2S_data_stream.read_amr_file(FLAGS.test_bpe_path, True)
    else:
        devset, tst_node, tst_in_neigh, tst_out_neigh, tst_sent = G2S_data_stream.read_amr_file(FLAGS.test_path, False)
    print('Number of dev samples: {}'.format(len(devset)))

    if FLAGS.finetune_path != "":
        print('Loading finetune set.')
        if FLAGS.use_bpe:
            ftset, ft_node, ft_in_neigh, ft_out_neigh, ft_sent = G2S_data_stream.read_amr_file(FLAGS.finetune_bpe_path, True)
        else:
            ftset, ft_node, ft_in_neigh, ft_out_neigh, ft_sent = G2S_data_stream.read_amr_file(FLAGS.finetune_path, False)
...
#         exit(0)  ## test train output
        ...
        for step in xrange(max_steps):
            ...
            if FLAGS.mode == 'rl_train':
                loss_value = train_graph.run_rl_training_subsample(sess, cur_batch, FLAGS, current_step=step, decay_steps=max_steps)
            elif FLAGS.mode == 'ce_train':
                loss_value, lr = train_graph.run_ce_training(sess, cur_batch, FLAGS, current_step=step, decay_steps=max_steps)
            total_loss += loss_value

            if step % 100==0:
                print('{0}:{1} '.format(step, lr), end="")
                sys.stdout.flush()
...
def validate_and_save(sess, saver, FLAGS, log_file,
    ...
    if valid_graph.mode == 'evaluate':
        ...
        dev_accu1 = res_dict['dev_accu1']
        dev_right1 = int(res_dict['dev_right1'])
        print('Dev accu = %.4f %d/%d Dev accu1 = %.4f %d/%d' % (dev_accu, dev_right, dev_total, dev_accu1, dev_right1, dev_total))
        log_file.write('Dev accu = %.4f %d/%d Dev accu1 = %.4f %d/%d\n' % (dev_accu, dev_right, dev_total, dev_accu1, dev_right1, dev_total))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device_id', type=str, help='cuda device set.')
    FLAGS, unparsed = parser.parse_known_args()

    device_id = FLAGS.device_id
    config_path = FLAGS.config_path

    os.environ["CUDA_VISIBLE_DEVICES"] = FLAGS.device_id # '4'
