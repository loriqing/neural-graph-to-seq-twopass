...
class Hypothesis(object):
    ...
    def extend(self, token, log_prob, attn_i, state, context_vector, coverage_vector=None):
        return Hypothesis(self.tokens + [token], self.log_probs + [log_prob], self.attn_ids + [attn_i], self.state + state,
...
def run_beam_search(sess, model, vocab, batch, options):
    ...
    hyps = []
    if options.pos_mask:
        hyps.append(Hypothesis([batch.sent_pos_inp[0][0]], [0.0], [-1], [initial_state], context_t, coverage_vector=coverage_t))
    else:
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        for h in hyps:
            cur_state_t_1.append(h.latest_state())
        ...
        model.decode_flag = True  # the first decode pass
        ...
        for h in sort_hyps(all_hyps):
            ...
            # hasn't reached stop token, so continue to extend this hypothesis
                ...
                hyps.append(h)
            if len(hyps) == options.beam_size or len(results) == options.beam_size:  # only choose the topk h
                break
    ...
    hyps_sorted_one = sort_hyps(results)

    # print('decoder2')
    best_hyps = hyps_sorted_one[0]
    decoder1_token = best_hyps.tokens[:-1]  # when train the second decode i feed the answer_pos_inp
    decoder1_state = best_hyps.state[1:]  # when train the second decode i feed the hidden state without init state
    decoder1_hidden_state = [state.h for state in decoder1_state]
    decoder1_hidden_state = np.stack(decoder1_hidden_state, axis=1)  # [1, answer_len1, hidden_dim]
    decoder1_cell_state = [state.c for state in decoder1_state]
    decoder1_cell_state = np.stack(decoder1_cell_state, axis=1)  # [1, answer_len1, hidden_dim]

    # get decoder_states and decoder_features and init_decoder2_state
    feed_dict = {}
    feed_dict[model.init_decoder_state] = initial_state
    feed_dict[model.decoder_c] = decoder1_cell_state
    feed_dict[model.decoder_h] = decoder1_hidden_state
    decoder1_states, decoder1_features, initial_state2 = sess.run([model.decoder1_states, model.decoder1_features, model.init_decoder2_state], feed_dict)
    # if options.reuse_decoder_state:
    #     initial_state2 = initial_state
    # else:
    #     de1_c = tf.reduce_sum(decoder1_cell_state, axis=1)
    #     de1_h = tf.reduce_sum(decoder1_hidden_state, axis=1)
    #     initial_state2 = tf.contrib.rnn.LSTMStateTuple(de1_c, de1_h)

    answer1_nodes = np.array(decoder1_token)[np.newaxis, :]  # [1, decoder1_token_len]
    answer1_nodes_size = [len(decoder1_token)]
    answer1_nodes_size_max = len(decoder1_token)
    answer1_nodes_mask = sess.run(tf.sequence_mask(answer1_nodes_size, answer1_nodes_size_max, dtype=tf.float32)) # [1, decoder1_token_len]
    # Initialize this first hypothesis
    context_t = np.zeros([model.encoder_dim + model.decoder1_dim])  # [encoder_dim + decode_dim]
    coverage_t = np.zeros((encoder_states.shape[1]))  # [passage_len]
    hyps = []
    hyps.append(Hypothesis([batch.sent_inp[0][0]], [0.0], [-1], [initial_state2], context_t, coverage_vector=coverage_t))
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        cur_encoder_states = np.tile(encoder_states, (cur_size, 1, 1))
        cur_encoder_features = np.tile(encoder_features,(cur_size, 1, 1))  # [batch_size,passage_len, options.attention_vec_size]
        cur_decoder1_states = np.tile(decoder1_states, (cur_size, 1, 1))
        cur_decoder1_features = np.tile(decoder1_features, (cur_size, 1, 1))  # [batch_size,passage_len, options.attention_vec_size]
        cur_nodes_idx = np.tile(node_idx, (cur_size, 1))  # [batch_size, passage_len]
        cur_nodes_mask = np.tile(node_mask, (cur_size, 1))  # [batch_size, passage_len]
        cur_decode1_idx = np.tile(answer1_nodes, (cur_size, 1))  # [batch_size, answer_len]
        cur_decode1_mask = np.tile(np.array(answer1_nodes_mask), (cur_size, 1))  # [batch_size, answer_len]
        cur_state_t_1 = []  # [2, gen_steps]
        cur_context_t_1 = []  # [batch_size, encoder_dim]
        cur_coverage_t_1 = []  # [batch_size, anwser1_len]
        cur_word_t = []  # [batch_size]
        for h in hyps:
            cur_state_t_1.append(h.latest_state())
            cur_context_t_1.append(h.context_vector)
        ...
        feed_dict = {}
        feed_dict[model.init_decoder2_state] = new_dec_init_state
        feed_dict[model.context2_t_1] = cur_context_t_1
        feed_dict[model.coverage2_t_1] = cur_coverage_t_1
        feed_dict[model.word2_t] = cur_word_t
        feed_dict[model.encoder_states] = cur_encoder_states
        feed_dict[model.encoder_features] = cur_encoder_features
        feed_dict[model.decoder1_states] = cur_decoder1_states
        feed_dict[model.decoder1_features] = cur_decoder1_features
        feed_dict[model.nodes] = cur_nodes_idx
        feed_dict[model.nodes_mask] = cur_nodes_mask
        feed_dict[model.decode1_idxs] = cur_decode1_idx
        feed_dict[model.decode1_mask] = cur_decode1_mask

        (state_t, context_t, attn_dist_t, coverage_t, topk_log_probs, topk_ids) = sess.run([model.state2_t, model.context2_t, model.attn_dist2_t,
                                                                            model.coverage2_t, model.topk_log_probs2, model.topk_ids2], feed_dict)
    ...
    # Sort hypotheses by average log probability
    hyps_sorted_two = sort_hyps(results)

    # Return the hypothesis with highest average log prob
    return hyps_sorted_one, hyps_sorted_two
...
if __name__ == '__main__':
    ...
    if FLAGS.use_bpe:
        word_vocab = Vocab(FLAGS.bpe_vec_path, fileformat='txt2')
    else:
    ...
    if FLAGS.use_bpe:
        testset, _, _, _, _ = G2S_data_stream.read_amr_file(in_path, True)
    else:
        testset, _, _, _, _ = G2S_data_stream.read_amr_file(in_path, False)
    ...
    with tf.Graph().as_default():
        ...
        if mode.endswith('evaluate'):
            ...
            outfile1 = open(out_path+'1', 'wt')
            outfile2 = open(out_path+'2', 'wt')
        ...
        for i in range(total_num):
            ...
            if mode in ['greedy', 'multinomial']:
                ...
                for j in xrange(cur_batch.batch_size):
                    outfile1.write(cur_batch.target_ref[j].encode('utf-8') + "\n")
                    outfile1.write(sentences[j].encode('utf-8') + "\n")
                    outfile1.write("========\n")
                outfile1.flush()
            else: # beam search
                print('Instance {}'.format(i))
                hyps_one, hyps_two = run_beam_search(sess, valid_graph, word_vocab, cur_batch, FLAGS)
                cur_passage = cur_batch.amr_node[0]

                outfile1.write(cur_batch.id[0] + "\n")
                outfile1.write(' '.join(cur_batch.target_ref[0]).encode('utf-8') + "\n")
                for j in xrange(1):
                    hyp = hyps_one[j]
                    cur_sent = hyp.idx_seq_to_string(cur_passage, word_vocab, FLAGS)
                    outfile1.write(cur_sent.encode('utf-8') + "\n")
                outfile1.write("--------\n")
                outfile1.write("========\n")
                outfile1.flush()

                outfile2.write(cur_batch.id[0] + "\n")
                outfile2.write(' '.join(cur_batch.target_ref[0]).encode('utf-8') + "\n")
                for j in xrange(1):
                    hyp = hyps_two[j]
                    cur_sent = hyp.idx_seq_to_string(cur_passage, word_vocab, FLAGS)
                    outfile2.write(cur_sent.encode('utf-8') + "\n")
                outfile2.write("--------\n")
                outfile2.write("========\n")
                outfile2.flush()
        if mode.endswith('evaluate'):
