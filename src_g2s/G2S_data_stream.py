...
class G2SBatch(object):
    def __init__(self, instances, options, word_vocab=None):
        ...
        self.target_ref = [x[-4] for x in instances] # list of tuples
        ...
        self.sent_pos_inp = []
        self.sent_pos_out = []
        for _, _, _, _, _, _, sentence_idx, sentence, sentence_pos_idx, sentence_pos, id in instances:
            if len(sentence_idx) < options.max_answer_len:
                ...
                self.sent_pos_inp.append([start_id, ]+sentence_pos_idx)
                self.sent_pos_out.append(sentence_pos_idx+[end_id, ])
            else:
                self.sent_inp.append([start_id,]+sentence_idx[:-1])
                self.sent_out.append(sentence_idx)
                self.sent_pos_inp.append([start_id, ]+sentence_pos_idx[:-1])
                self.sent_pos_out.append(sentence_pos_idx)
