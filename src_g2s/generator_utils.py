...
class CovCopyAttenGen:
    ...
    def one_step_decoder2(self, state_t_1, context_t_1, coverage_t_1, word_t, encoder_states, encoder_features, decoder1_states, decoder1_features,
                         node_idxs, node_mask, decode1_idxs, decode1_mask, v1, v2, w_c1, w_c2, vocab):
        '''
        ...
        with variable_scope.variable_scope("attention_encoder1"):
            context_t1, attn_dist, coverage_t = self.attention(state_t, options.attention_vec_size, encoder_states,
                                                             encoder_features, node_mask, v1, w_c=w_c1,
                                                             use_coverage=options.use_coverage, coverage=coverage_t_1)
        with variable_scope.variable_scope("attention_decoder2"):
            # this attention don't need coverage and pointer-network ???
            context_t2, _, _ = self.attention(state_t, options.attention_vec_size, decoder1_states,
                                                      decoder1_features, decode1_mask, v2, w_c=w_c2,
                                                      coverage=None)
        # Calculate p_gen, Equation (8)
        p_gen = None
        if options.pointer_gen:
            with tf.variable_scope('calculate_pgen'):
                p_gen = linear([context_t1, state_t.c, state_t.h, x], 1, True) # [batch_size, 1]
                p_gen = tf.sigmoid(p_gen)

        # Concatenate the cell_output (= decoder state) and the context vector, and pass them through a linear layer
        # This is V[s_t, h*_t] + b in the paper
        context_t = tf.concat([context_t1,context_t2], 1)  # [batch_size, encoder_dim + decoder_dim]
        with variable_scope.variable_scope("AttnOutputProjection"):
            output_t = linear([cell_output] + [context_t], options.gen_hidden_size, True)
    ...
    def train_mode(self, vocab, encoder_dim, encoder_states, encoder_features, node_idxs, node_mask,
        ...
        loss_weights: [batch_size, gen_steps]
        '''
        options = self.options
        ...
        if mode_gen == 'ce_loss': reuse = None
        elif mode_gen == 'greedy': reuse = True
        with variable_scope.variable_scope("attention_decoder1", reuse=reuse):
            # print('attention_decoder1')
            ...
            for i in range(options.max_answer_len):
                ...
                decoder_cell_state.append(state_t.c)  # prepare for the second decoder
                decoder_hidden_state.append(state_t.h)  # prepare for the second decoder
        ...
        decoder_cell_state = tf.multiply(tf.stack(decoder_cell_state, axis=1), tf.expand_dims(loss_weights, axis=-1)) # [batch_size， max_dec_steps, gen_hidden_dim]
        decoder_hidden_state = tf.multiply(tf.stack(decoder_hidden_state, axis=1), tf.expand_dims(loss_weights, axis=-1)) # [batch_size, max_dec_steps, gen_hidden_dim]
        ...
        if mode_gen == 'ce_loss':
            ...
            accuracy, vocab_scores = _mask_and_accuracy(vocab_scores, answer_batch, loss_weights)
            return accuracy, vocab_scores, self._loss, sampled_words, decoder_cell_state, decoder_hidden_state

        else:
            return None, self._loss, sampled_words, decoder_cell_state, decoder_hidden_state

    def train_mode_two_pass(self, vocab, encoder_dim, encoder_states, encoder_features, decoder1_dim, decoder1_states, decoder1_features,
                            node_idxs, node_mask, decode1_idxs, decode1_mask, init_state, decoder_inputs, answer_batch, loss_weights,
                            loss1, accuracy1, mode_gen='ce_loss'):
        '''
        encoder_dim: int-valued
        encoder_states: [batch_size, answer, encoder_dim], for calculating context vector from the first decoder
        encoder_features: [batch_size, anwers, feature_dim], the encoder part for calculating attention (W*s) from the first decoder
        node_idxs: [batch_size, passage_len] int32
        node_mask: [batch_size, passage_len] 0/1
        init_state: Tuple of [batch_size, gen_hidden_size]
        decoder_inputs: [batch_size, max_dec_steps]
        answer_batch: [batch_size, max_dec_steps]
        loss1: loss of the first decoder
        accuracy1: accuracy of the first decoder
        '''

        options = self.options

        input_shape = tf.shape(decoder1_states)
        batch_size = input_shape[0]
        ...
        # initialize all the variables
        state_t_1 = init_state
        context_t_1 = tf.zeros([batch_size, encoder_dim + decoder1_dim])
        coverage_t_1 = None
        ...
        vocab_scores = []
        sampled_words = []
        self.decoder1_features = decoder1_features
        with variable_scope.variable_scope("attention_decoder2"):
            # print('attention_decoder2')
            # Get the weight vectors v and W_c (W_c is for coverage)  # how to reuse the v and w_c in decoder2 ??
            v1 = variable_scope.get_variable("v1", [options.attention_vec_size])
            v1 = tf.expand_dims(tf.expand_dims(v1, axis=0), axis=0)
            v2 = variable_scope.get_variable("v2", [options.attention_vec_size])
            v2 = tf.expand_dims(tf.expand_dims(v2, axis=0), axis=0)
            w_c1 = None
            w_c2 = None
            if options.use_coverage:
                with variable_scope.variable_scope("coverage"):
                    w_c1 = variable_scope.get_variable("w_c1", [options.attention_vec_size])
                    w_c1 = tf.expand_dims(tf.expand_dims(w_c1, axis=0), axis=0)
                    w_c2 = variable_scope.get_variable("w_c2", [options.attention_vec_size])
                    w_c2 = tf.expand_dims(tf.expand_dims(w_c2, axis=0), axis=0)
            ...
            for i in range(options.max_answer_len):
                ...
                (state_t, context_t, coverage_t, attn_dist_t, p_gen_t, output_t) = self.one_step_decoder2(
                                state_t_1, context_t_1, coverage_t_1, word_t, encoder_states, encoder_features,
                                decoder1_states, self.decoder1_features, node_idxs, node_mask, decode1_idxs, decode1_mask,
                                v1, v2, w_c1, w_c2, vocab)
                coverages.append(coverage_t)
                attn_dists.append(attn_dist_t)
                p_gens.append(p_gen_t)
                vocab_scores.append(output_t) # The vocabulary distributions.
        ...
        if len(sampled_words)!=0:
            sampled_words2 = tf.stack(sampled_words, axis=1) # [batch_size, max_dec_steps]
        ...
        if mode_gen in ('ce_loss', 'rl_loss', ):
            ...
            if loss1 is not None:
                self._loss = options.two_loss_wt * self._loss + (1 - options.two_loss_wt) * loss1
        # accuracy is calculated only under 'ce_loss' mode, where true answer is given
        if mode_gen == 'ce_loss':
            # The answer_batch in rl_train is actually sampled seq, not gold seq.
            # So accuracy is calculated only in 'ce_loss' mode
            if accuracy1 is None:
                print('error! accuracy1 is None')
                exit(0)
            accuracy2, vocab_scores = _mask_and_accuracy(vocab_scores, answer_batch, loss_weights)
            accuracy = [accuracy1, accuracy2]
            return accuracy, vocab_scores, self._loss, sampled_words2
        else:
            return None, self._loss, sampled_words2

    def calculate_encoder_features(self, encoder_states, encoder_dim):
        print('calculate_encoder_features')
        options = self.options
        input_shape = tf.shape(encoder_states)
        batch_size = input_shape[0]
        passage_len = input_shape[1]
        with variable_scope.variable_scope("attention_decoder1"):
    ...
    def calculate_decoder_features(self, decoder_states, decoder_dim):
        print('calculate_decoder_features')
        options = self.options
        input_shape = tf.shape(decoder_states)
        batch_size = input_shape[0]
        answer_len = input_shape[1]
        with variable_scope.variable_scope("attention_decoder2"):
            decoder_features = tf.expand_dims(decoder_states, axis=2) # now is shape [batch_size, answer_len, 1, decoder_dim]
            W_h = variable_scope.get_variable("W_h", [1, 1, decoder_dim, options.attention_vec_size])
            self.W_h = W_h
            decoder_features = nn_ops.conv2d(decoder_features, W_h, [1, 1, 1, 1], "SAME") # [batch_size, answer_len, 1, attention_vec_size]
            decoder_features = tf.reshape(decoder_features, [batch_size, answer_len, options.attention_vec_size])
    ...
    def decode_mode_two_pass(self, word_vocab, beam_size, state_t_1, context_t_1, coverage_t_1, word_t,
                             encoder_states, encoder_features, node_idxs, node_mask,
                             decoder1_states, decoder1_features, decode1_idxs, decode1_mask):
        options = self.options
        ...
        with variable_scope.variable_scope("attention_decoder2"):
            ...
            (state_t, context_t, coverage_t, attn_dist_t, p_gen_t, output_t) = self.one_step_decoder2(
                                state_t_1, context_t_1, coverage_t_1, word_t_representation, encoder_states, encoder_features, decoder1_states, decoder1_features,
                                node_idxs, node_mask, decode1_idxs, decode1_mask, v1, v2, w_c1, w_c2, word_vocab)
            vocab_scores = tf.log(output_t)
            greedy_prediction = tf.reshape(tf.argmax(output_t, 1),[-1]) # calcualte greedy
            multinomial_prediction = tf.reshape(tf.multinomial(vocab_scores, 1),[-1]) # calculate multinomial
            topk_log_probs, topk_ids = tf.nn.top_k(vocab_scores, beam_size) # calculate topK
...
# loss_weights: [batch_size, max_answer_len]
def _mask_and_accuracy(values, answers, loss_weights):
    values = tf.argmax(values,axis=2)  # [batch_size, step_size]
    ...
    return tf.reduce_sum(res), values
